...
import { Alert, ViewController, LoadingController, ToastController, Content, AlertController, Platform, NavController, PopoverController, IonicPage, ModalController, NavParams } from 'ionic-angular';
...
export class Page1 implements OnInit {
  ...
  constructor(private device: Device,
    private viewCtrl: ViewController, private toast: ToastController,// private network: Network,
  ...
  findCityPosts() { 
    ...
    return CityPosts.find({ cityname: this.pcity, citylang: this.plang, deleted: false, approved: true }, { sort: { upDate: -1 } }).map(cityposts => {
      ...
      cityposts.forEach(citypost => {
        ...
        citypost.posterpicture = Pictures.getPictureUrl(this.profile.pictureId, platform);
  ...
  citypost(): void {
    ...
    const modal = this.modalCtrl.create('CityPostPage');
    modal.present();
