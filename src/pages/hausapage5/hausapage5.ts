...
@Component({
  selector: 'hausapage5',
  templateUrl: 'hausapage5.html'
})
export class HausaPage5 implements OnInit { 
  ...
  constructor(private viewCtrl: ViewController ,    private platform: Platform, public modalCtrl: ModalController, private alertCtrl: AlertController,
   ...
   // console.log(this.pcity)
    ...
    subscribePosts(): Subscription {   
          ...
          findPosts() {
            ...
            return SearchCitys.find({citylang: 'Hausa'},{limit: 20, sort: {alltext: 1} }) 
            .startWith([]);
          }
        ...
        if(type=="cpost")
          {
            ...
            let citypost =  CityPosts.findOne({_id:id })
              
            console.log(citypost)

              
            const modal = this.modalCtrl.create('HausaPostPage', {citypost});
            modal.present();
